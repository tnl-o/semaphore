...
func (c *ProjectsController) AddProject(w http.ResponseWriter, r *http.Request) {
	...
	log.Info(fmt.Sprintf("Creating project: name=%s, user_id=%d, admin=%v", body.Name, user.ID, user.Admin))
	body, err := store.CreateProject(body)
	if err != nil {
		log.Error(fmt.Sprintf("Failed to create project: %v", err))
		helpers.WriteError(w, err)
		return
	}
	log.Info(fmt.Sprintf("Project created successfully: id=%d, name=%s", body.ID, body.Name))
	...
	if err != nil {
		log.Error(fmt.Sprintf("Failed to create environment for project %d: %v", body.ID, err))
		helpers.WriteError(w, err)
		return
	}

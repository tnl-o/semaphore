//! Template Exporter
//!
//! Экспорт шаблонов

use crate::error::{Error, Result};
use crate::models::Template;
use crate::db::store::Store;
use super::exporter_main::{TypeExporter, DataExporter, ValueMap, Progress};

/// Экспортёр шаблонов
pub struct TemplateExporter {
    /// Карта значений
    pub value_map: ValueMap<Template>,
}

impl TemplateExporter {
    /// Создаёт новый экспортёр
    pub fn new() -> Self {
        Self {
            value_map: ValueMap::new(),
        }
    }
}

impl Default for TemplateExporter {
    fn default() -> Self {
        Self::new()
    }
}

impl TypeExporter for TemplateExporter {
    /// Загружает данные
    fn load(&mut self, store: &dyn Store, exporter: &dyn DataExporter, _progress: Progress) -> Result<()> {
        // Получаем ключи проектов
        let projs = exporter.get_loaded_keys_int("Project", "global")?;

        for proj in projs {
            let templates = store.get_templates(proj)
                .map_err(|e| Error::Other(format!("Failed to load templates: {}", e)))?;

            for template in templates {
                self.value_map.append_value(template, proj.to_string())?;
            }
        }

        Ok(())
    }

    /// Восстанавливает данные
    fn restore(&mut self, store: &dyn Store, exporter: &dyn DataExporter, _progress: Progress) -> Result<()> {
        for val in &self.value_map.values {
            let mut old = val.value.clone();

            // Восстанавливаем ссылки
            old.project_id = exporter.get_new_key_int("Project", "global", old.project_id, self)?;
            old.inventory_id = exporter.get_new_key_int("Inventory", &val.scope, old.inventory_id, self)?;
            old.repository_id = exporter.get_new_key_int("Repository", &val.scope, old.repository_id, self)?;
            old.environment_id = exporter.get_new_key_int("Environment", &val.scope, old.environment_id, self)?;

            let new_template = store.create_template(old)
                .map_err(|e| Error::Other(format!("Failed to create template: {}", e)))?;

            exporter.map_int_keys(self.get_name(), &val.scope, val.value.id, new_template.id)?;
        }

        Ok(())
    }

    /// Получает имя
    fn get_name(&self) -> &str {
        "Template"
    }

    /// Получает зависимости экспорта
    fn export_depends_on(&self) -> Vec<&str> {
        vec!["Project"]
    }

    /// Получает зависимости импорта
    fn import_depends_on(&self) -> Vec<&str> {
        vec!["Project", "Inventory", "Repository", "Environment"]
    }

    /// Получает ошибки
    fn get_errors(&self) -> Vec<String> {
        self.value_map.errors.clone()
    }

    /// Очищает
    fn clear(&mut self) {
        self.value_map.clear();
    }
}

// ============================================================================
// Tests
// ============================================================================

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_template_exporter_creation() {
        let exporter = TemplateExporter::new();
        assert_eq!(exporter.get_name(), "Template");
    }

    #[test]
    fn test_template_exporter_depends_on() {
        let exporter = TemplateExporter::new();
        assert_eq!(exporter.export_depends_on(), vec!["Project"]);
    }
}

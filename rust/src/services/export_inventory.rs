//! Inventory Exporter
//!
//! Экспорт инвентарей

use crate::error::{Error, Result};
use crate::models::Inventory;
use crate::db::store::Store;
use super::exporter_main::{TypeExporter, DataExporter, ValueMap, Progress};

/// Экспортёр инвентарей
pub struct InventoryExporter {
    /// Карта значений
    pub value_map: ValueMap<Inventory>,
}

impl InventoryExporter {
    /// Создаёт новый экспортёр
    pub fn new() -> Self {
        Self {
            value_map: ValueMap::new(),
        }
    }
}

impl Default for InventoryExporter {
    fn default() -> Self {
        Self::new()
    }
}

impl TypeExporter for InventoryExporter {
    /// Загружает данные
    fn load(&mut self, store: &dyn Store, exporter: &dyn DataExporter, _progress: Progress) -> Result<()> {
        // Получаем ключи проектов
        let projs = exporter.get_loaded_keys_int("Project", "global")?;

        for proj in projs {
            let inventories = store.get_inventories(proj)
                .map_err(|e| Error::Other(format!("Failed to load inventories: {}", e)))?;

            for inventory in inventories {
                self.value_map.append_value(inventory, proj.to_string())?;
            }
        }

        Ok(())
    }

    /// Восстанавливает данные
    fn restore(&mut self, store: &dyn Store, exporter: &dyn DataExporter, _progress: Progress) -> Result<()> {
        for val in &self.value_map.values {
            let mut old = val.value.clone();

            // Восстанавливаем ссылки
            old.project_id = exporter.get_new_key_int("Project", "global", old.project_id, self)?;
            old.ssh_key_id = exporter.get_new_key_int_ref("AccessKey", &val.scope, old.ssh_key_id, self)?;
            old.become_key_id = exporter.get_new_key_int_ref("AccessKey", &val.scope, old.become_key_id, self)?;
            old.repository_id = exporter.get_new_key_int_ref("Repository", &val.scope, old.repository_id, self)?;

            let new_inventory = store.create_inventory(old)
                .map_err(|e| Error::Other(format!("Failed to create inventory: {}", e)))?;

            exporter.map_int_keys(self.get_name(), &val.scope, val.value.id, new_inventory.id)?;
        }

        Ok(())
    }

    /// Получает имя
    fn get_name(&self) -> &str {
        "Inventory"
    }

    /// Получает зависимости экспорта
    fn export_depends_on(&self) -> Vec<&str> {
        vec!["Project"]
    }

    /// Получает зависимости импорта
    fn import_depends_on(&self) -> Vec<&str> {
        vec!["Project", "AccessKey", "Repository"]
    }

    /// Получает ошибки
    fn get_errors(&self) -> Vec<String> {
        self.value_map.errors.clone()
    }

    /// Очищает
    fn clear(&mut self) {
        self.value_map.clear();
    }
}

// ============================================================================
// Tests
// ============================================================================

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_inventory_exporter_creation() {
        let exporter = InventoryExporter::new();
        assert_eq!(exporter.get_name(), "Inventory");
    }

    #[test]
    fn test_inventory_exporter_depends_on() {
        let exporter = InventoryExporter::new();
        assert_eq!(exporter.export_depends_on(), vec!["Project"]);
    }
}
